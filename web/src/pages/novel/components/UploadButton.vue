...
async function beforeUpload({ file }: { file: UploadFileInfo }) {
  ...
  if (
    ...
    message.error('Do not upload machine translation files generated on this website');
    return false;
  }
  ...
  if (charsCount.total < 500) {
    message.error('Too few characters, please check if the content is an image');
    return false;
  }
  ...
  if (p < 0.33) {
    if (!props.allowZh) {
      message.error('Suspected Chinese novel, light novel library does not allow upload');
      return false;
    } else {
      file.url = 'zh';
    }
...
<template>
  ...
  >
    ...
    <n-ul>
      <n-li>
        Before uploading Japanese chapters, please make sure there is text inside. Single volume compressed packages over 40MB likely only contain scanned images without text, which cannot be translated.
      </n-li>
              <n-li>EPUB files larger than 40MB cannot be uploaded, please compress the images inside.</n-li>
              <n-li>Do not upload existing volumes, if there are issues with existing volumes please contact the administrator.</n-li>
              <n-li>Volume file names should only contain Japanese title, volume number, and volume Japanese title.</n-li>
    </n-ul>
